...
#pragma omp parallel for reduction(max:r) schedule(static)
    for (long long k = 0; k < static_cast<long long>(m.a.size()); ++k) {
        r = std::max(r, std::abs(m.a[static_cast<size_t>(k)]));
    }
    return r;
}
...
#pragma omp parallel for reduction(+:s) schedule(static)
    for (long long k = 0; k < static_cast<long long>(m.a.size()); ++k) s += m.a[static_cast<size_t>(k)];
    return s / static_cast<double>(m.a.size());
}
