/*
 * Pure MPI domain-decomposition lid-driven cavity solver.
 *
 * This is a true domain-decomposition example with vectorized/SIMD-friendly local kernels: one CFD domain is split in the
 * vertical direction across MPI ranks. Each rank owns a block of rows and
 * exchanges ghost rows with neighbouring ranks. This folder uses MPI only;
 * the hybrid MPI+OpenMP version is kept separately in cpp/hybrid_mpi_openmp/.
 *
 ...
 * driver; this folder is the pure MPI domain decomposition solver.
 */
...
static Config parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--N") cfg.N = std::stoi(need_value(a));
            ...
            std::cout << "Pure MPI domain-decomposition lid-driven cavity solver\n"
                      << "Options:\n"
...
static void gather_and_write_fields(
    ...
    if (d.rank == 0) {
        ...
        name << cfg.out_dir << "/cpp_mpi_domain_vectorized_N" << cfg.N << "_Re" << cfg.Re << "_" << cfg.scheme << "_fields.csv";
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (rank == 0) {
            ...
            summary_path << cfg.out_dir << "/cpp_mpi_domain_vectorized_summary.csv";
            const bool exists = fs::exists(summary_path.str());
            std::ofstream summary(summary_path.str(), std::ios::app);
            if (!exists) {
                summary << "Implementation,N,Re,Scheme,MPI_Ranks,Steps,PoissonIters,FinalDt,FinalPsiChange,FinalOmegaChange,Runtime_s,DomainDecomposition\n";
            }
            summary << "cpp_mpi_domain_vectorized," << cfg.N << ',' << cfg.Re << ',' << cfg.scheme << ','
                    << size << ',' << cfg.steps << ',' << cfg.poisson_iters << ','
                    << final_dt << ',' << final_psi_change << ',' << final_omega_change << ','
                    << runtime << ",true\n";
            std::cout << "Pure MPI domain-decomposition run finished in " << runtime << " s\n";
