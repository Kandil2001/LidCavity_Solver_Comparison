...
static void parse_args(Config* cfg, int argc, char** argv) {
    ...
    for (int i=1; i<argc; ++i) {
        ...
        if (str_eq(a, "--N")) cfg->N = atoi(need_value(&i, argc, argv, a));
            ...
            printf("c_openmp_domain_looped OpenMP domain solver\n");
...
static void write_summary(const Config* cfg, double runtime, double omega_change, double psi_change, double max_u) {
    ...
    char path[1024];
    snprintf(path, sizeof(path), "%s/c_openmp_domain_looped_summary.csv", cfg->out_dir);
    FILE* f = fopen(path, "w");
...
static void write_fields(const Config* cfg, const double* psi, const double* omega) {
    ...
    ensure_dir(cfg->out_dir);
    char path[1024];
    snprintf(path, sizeof(path), "%s/c_openmp_domain_looped_N%d_Re%d_%s_fields.csv", cfg->out_dir, N, cfg->Re, cfg->scheme);
    FILE* f = fopen(path, "w");
...
int main(int argc, char** argv) {
    ...
    printf("c_openmp_domain_looped: N=%d Re=%d scheme=%s steps=%d runtime=%.6f s omega_change=%.3e psi_change=%.3e\n", cfg.N, cfg.Re, cfg.scheme, cfg.steps, runtime, omega_change, psi_change);
