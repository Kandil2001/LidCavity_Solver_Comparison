...
def main() -> int:
    ...
    parser.add_argument("--prefix", default="mpi_cpp_case_parallel")
    ...
    for path in sorted(args.raw.glob("rank_*/case_*/results/data/study_summary_single.csv")):
        ...
        df["Implementation"] = args.prefix + "_" + df["OriginalImplementation"].astype(str).str.replace("serial_", "", regex=False)
        frames.append(df)
    if not frames:
