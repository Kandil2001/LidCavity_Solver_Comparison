...
SOLVER_DIR="$ROOT_DIR/$SRC_ROOT/cpp/mpi_domain_${KERNEL_STYLE}"
...
make -C "$SOLVER_DIR" run \
    NP="$NP" N="$N" RE="$RE" STEPS="$STEPS" POISSON_ITERS="$POISSON_ITERS" \
    SCHEME="$SCHEME" POISSON_SOLVER="$POISSON_SOLVER" SOR_OMEGA="$SOR_OMEGA" \
    NO_FIELDS="$NO_FIELDS"
