#!/usr/bin/env python3
"""Run OpenMP scaling checks and create scaling CSV/plots.

Old case-level MPI sweep scaling was removed from the cleaned package.
For real MPI-domain timing, use scripts/run_domain_solver_benchmark.sh.
...
def target_specs(root: Path, python_exe: str) -> Dict[str, TargetSpec]:
    """Scaling targets kept for the original OpenMP benchmark family.

    Old case-level MPI sweep targets were removed from the cleaned package.
    For real MPI-domain timing, use scripts/run_domain_solver_benchmark.sh.
    """
    return {
        "c_openmp": TargetSpec(
