...
/*
 * Pure C MPI domain-decomposition lid-driven cavity solver.
 * Kernel style: explicit loop-based local kernels.
 *
...
static void parse_args(Config* cfg, int argc, char** argv, MPI_Comm comm) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (str_eq(a, "--N")) cfg->N = atoi(need_value(&i, argc, argv, a, comm));
            ...
            MPI_Comm_rank(comm, &rank);
            if (rank == 0) {
                printf("Pure C MPI domain-decomposition lid-driven cavity solver\n");
                printf("Options:\n");
...
static void write_summary(const Config* cfg, const Decomp* d, double runtime, double omega_change, double psi_change, double umax, int steps_done) {
    ...
    char path[1024];
    snprintf(path, sizeof(path), "%s/%s_summary.csv", cfg->out_dir, "c_mpi_domain_looped");
    FILE* f = fopen(path, "w");
    ...
    fprintf(f, "c_mpi_domain_looped,%d,%d,%s,%s,%.3f,%d,%d,%d,%d,%.10f,%.10e,%.10e,%.10e\n",
            cfg->N, cfg->Re, cfg->scheme, cfg->poisson_solver, cfg->sor_omega, steps_done, cfg->poisson_iters, d->size, 1, runtime, omega_change, psi_change, umax);
    fclose(f);
}
...
static void write_fields(const Config* cfg, const Decomp* d, const double* psi, const double* omega, const double* u, const double* v, MPI_Comm comm) {
    ...
    if (d->rank == 0) {
        ensure_dir_rank0(cfg->out_dir, 0);
        char path[1024];
        snprintf(path, sizeof(path), "%s/%s_N%d_Re%d_%s_fields.csv", cfg->out_dir, "c_mpi_domain_looped", cfg->N, cfg->Re, cfg->scheme);
        FILE* f = fopen(path, "w");
        if (f) {
