...
static void gather_and_write_fields(
    ...
    if (d.rank == 0) {
        ...
        name << cfg.out_dir << "/cpp_hybrid_mpi_openmp_vectorized_N" << cfg.N << "_Re" << cfg.Re << "_" << cfg.scheme << "_fields.csv";
...
int main(int argc, char** argv) {
    ...
    try {
        ...
        if (rank == 0) {
            ...
            summary_path << cfg.out_dir << "/cpp_hybrid_mpi_openmp_vectorized_summary.csv";
            ...
            #endif
            summary << "cpp_hybrid_mpi_openmp_vectorized," << cfg.N << ',' << cfg.Re << ',' << cfg.scheme << ','
