...
static void write_summary(const Config* cfg, const Decomp* d, double runtime, double omega_change, double psi_change, double umax, int steps_done) {
    ...
    char path[1024];
    snprintf(path, sizeof(path), "%s/%s_summary.csv", cfg->out_dir, "c_mpi_domain_vectorized");
    FILE* f = fopen(path, "w");
    ...
    fprintf(f, "c_mpi_domain_vectorized,%d,%d,%s,%s,%.3f,%d,%d,%d,%d,%.10f,%.10e,%.10e,%.10e\n",
...
static void write_fields(const Config* cfg, const Decomp* d, const double* psi, const double* omega, const double* u, const double* v, MPI_Comm comm) {
    ...
    if (d->rank == 0) {
        ensure_dir_rank0(cfg->out_dir, 0);
        char path[1024];
        snprintf(path, sizeof(path), "%s/%s_N%d_Re%d_%s_fields.csv", cfg->out_dir, "c_mpi_domain_vectorized", cfg->N, cfg->Re, cfg->scheme);
        FILE* f = fopen(path, "w");
        if (f) {
